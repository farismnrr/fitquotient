...
# Build and push to GitHub Container Registry (GHCR) for frontend (fitquotient-ui)
# Usage:
...
IMAGE_NAME=${IMAGE_NAME:-fitquotient-ui}
...
IMAGE_DESCRIPTION="FitQuotient UI — Next.js Frontend for FitQuotient platform"
...
# Optional build args for URL_CORE/URL_API_KEY
BUILD_ARGS=()
if [ -n "${URL_CORE:-}" ]; then
  BUILD_ARGS+=(--build-arg "URL_CORE=${URL_CORE}")
fi
if [ -n "${URL_API_KEY:-}" ]; then
  BUILD_ARGS+=(--build-arg "URL_API_KEY=${URL_API_KEY}")
fi
docker build -f "${DOCKERFILE}" -t "${IMAGE}" "${LABEL_FLAGS[@]}" "${BUILD_ARGS[@]}" .
